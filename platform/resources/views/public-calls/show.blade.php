@extends('layouts.app')

@php
    $title = $call->translated('title');
    $bodyLocale = $call->translationLocale('body');
    $fallbackLocale = $call->isTranslationFallback('title') ? $call->translationLocale('title') : null;
    $types = [
        'recruitment' => __('ui.recruitment'),
        'grant' => __('ui.grants'),
        'funding' => __('ui.funding'),
        'commission' => __('ui.commissions'),
    ];
@endphp

@section('title', $title)

@section('content')
    <section class="py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <a href="{{ route('public-calls.index') }}" class="text-[#014DA4] hover:text-[#013b7a] text-sm font-medium mb-6 inline-block">&larr; {{ __('ui.back_to_calls') }}</a>

            <article class="bg-white rounded-lg shadow-sm border border-gray-100 p-8">
                <div class="flex flex-wrap items-center gap-3 mb-4">
                    <span class="inline-block px-2 py-0.5 text-xs font-medium bg-[#c8a84e]/10 text-[#c8a84e] rounded">{{ $types[$call->type] ?? ucfirst($call->type) }}</span>
                    <span class="inline-block px-2 py-0.5 text-xs font-medium {{ $call->isOpen() ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }} rounded">
                        {{ $call->isOpen() ? __('ui.open') : __('ui.closed') }}
                    </span>
                    @if($fallbackLocale)
                        <span class="inline-block px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-500 rounded">
                            {{ __('ui.only_in_language', ['language' => \App\Support\Locales::label($fallbackLocale)]) }}
                        </span>
                    @endif
                </div>
                <h1 class="text-3xl font-bold text-gray-900 mb-4" @if($fallbackLocale) lang="{{ $fallbackLocale }}" @endif>{{ $title }}</h1>
                @if($call->deadline)
                    <p class="text-sm text-gray-500 mb-6">{{ __('ui.deadline') }}: {{ $call->deadline->format('F d, Y') }}</p>
                @endif
                <div class="prose max-w-none text-gray-600 mb-6" @if($bodyLocale && $bodyLocale !== app()->getLocale()) lang="{{ $bodyLocale }}" @endif>
                    {!! $call->translated('body') !!}
                </div>
                @if($call->attachment)
                    <a href="{{ asset('storage/' . $call->attachment) }}" class="inline-flex items-center gap-2 bg-[#32373c] hover:bg-[#23282d] text-white px-5 py-2.5 rounded transition" download>
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                        {{ __('ui.download_attachment') }}
                    </a>
                @endif
            </article>
        </div>
    </section>
@endsection
